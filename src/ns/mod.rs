// Standard (Rust-backed) namespaces, accessed with `.` syntax (e.g. operators.lap,
...
// functions (operators/solver) carry their own implementations and dispatch via a
...
pub fn is_ns_builtin(name: &str) -> bool {
    operators::NAMES.contains(&name) || solver::NAMES.contains(&name)
}
...
pub fn dispatch(name: &str, vals: Vec<Val>, env: &crate::eval::Env) -> Option<Result<Val, String>> {
    if operators::NAMES.contains(&name) { return Some(operators::dispatch(name, vals, env)); }
...
pub fn register_all(vars: &mut HashMap<String, Val>) {
    insert_ns(vars, "operators", operators::members());
