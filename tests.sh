...
run "ten.zeros_2d"   "zeros(2,3)"                       "$(printf '[0  0  0]\n[0  0  0]')"
run "ten.ones_2d"    "ones(2,2)"                        "$(printf '[1  1]\n[1  1]')"
run "ten.eye2"       "eye(2)"                           "$(printf '[1  0]\n[0  1]')"
run "ten.eye3_trace" "trace(eye(3))"                    "3"
run "ten.diag"       "diag((2,3,4))"                    "$(printf '[2  0  0]\n[0  3  0]\n[0  0  4]')"
...
run "ten.add_scalar"    "ones(2,2) + 1"                    "$(printf '[2  2]\n[2  2]')"
run "ten.sub_scalar"    "ones(2,2) * 3 - 1"               "$(printf '[2  2]\n[2  2]')"
run "ten.mul_scalar"    "eye(3) * 5"                       "$(printf '[5  0  0]\n[0  5  0]\n[0  0  5]')"
run "ten.div_scalar"    "ones(2,2) * 4 / 2"               "$(printf '[2  2]\n[2  2]')"
run "ten.add_tensor"    "eye(2) + eye(2)"                  "$(printf '[2  0]\n[0  2]')"
run "ten.sub_tensor"    "eye(2) - eye(2)"                  "$(printf '[0  0]\n[0  0]')"
run "ten.neg_tensor"    "-eye(2)"                          "$(printf '[-1   0]\n[ 0  -1]')"
run "ten.pow_scalar"    "ones(2,2) * 3 ^ 2"               "$(printf '[9  9]\n[9  9]')"

# ── Tensor operations ─────────────────────────────────────────────────────────
section "TENSORS - OPERATIONS"
run "ten.transpose"     "transpose(matrix(2,3,(i,j)->i*3+j))" "$(printf '[0  3]\n[1  4]\n[2  5]')"
run "ten.transpose_sq"  "transpose(eye(3))"                   "$(printf '[1  0  0]\n[0  1  0]\n[0  0  1]')"
run "ten.trace_eye"     "trace(eye(5))"                       "5"
run "ten.norm_eye3"     "norm(eye(3))"                        "1.7320508075688772"
run "ten.norm_ones"     "norm(ones(4))"                       "2"
run "ten.matmul_id"     "matmul(eye(2), eye(2))"              "$(printf '[1  0]\n[0  1]')"
run "ten.matmul_basic"  "matmul(matrix(1,2,(i,j)->j+1), matrix(2,1,(i,j)->i+1))" "$(printf '[5]')"
...
run "ten.map_tensor"    "map(x -> x*2, eye(2))"               "$(printf '[2  0]\n[0  2]')"
